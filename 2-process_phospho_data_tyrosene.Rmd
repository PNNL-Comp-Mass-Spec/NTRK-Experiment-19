...
data_package_num <- 4925
data_folder <- "data/phospho_data_tyrosine"
crosstab_prefix <- "ptrc_ex19_tyrosene_4925"
...
msnid_4924 <- read_msgf_data_from_DMS(4924)
show(msnid_4924)

fractions <- psms(msnid) %>%
  select(Dataset) %>% unique() %>%
  mutate(PlexID = sub("^PTRC_Exp19_Plex_0([12]+)_.*$", "\\1", Dataset))
write.table(fractions, "data/phospho_data/fractions_4925.txt", sep = "\t", quote = F, row.names = F)

```


```{r}
psms_tyrosene <- psms(msnid)
psms_4924 <- psms(msnid_4924)

print("peptides from 4924 shared with 4925")
table(psms_4924$pepSeq %in% psms_tyrosene$pepSeq)

print("Tyrosene in 4924")
table(grepl("Y\\*", psms_4924$peptide))
print("S and T in 4924")
table(grepl("S\\*", psms_4924$peptide) | grepl("T\\*", psms_4924$peptide))

print("Tyrosene in 4925")
table(grepl("Y\\*", psms_tyrosene$peptide))
print("S and T in 4925")
table(grepl("S\\*", psms_tyrosene$peptide) | grepl("T\\*", psms_tyrosene$peptide))
...
ascore_4924 <- get_AScore_results(4924)
msnid_4924 <- best_PTM_location_by_ascore(msnid_4924, ascore_4924)
msnid_4924 <- apply_filter(msnid_4924, "grepl(\"\\\\*\", peptide)")
...
write.table(quant_cross_tab, file=file.path(data_folder, paste(crosstab_prefix, "crosstab_tyrosine_4925_phospho_SiteID_original.txt", sep="_")),
            quote=F, sep="\t")
```
...
write.table(quant_cross_tab, file=file.path(data_folder, paste(crosstab_prefix, "crosstab_tyrisine_4925_phospho_SiteID_median_centered.txt", sep="_")),
            quote=F, sep="\t")
```
